...
public class GroupAgent {
		...
		for(int i=0; i < agents_.Count; i++){
			...
			if(delta <= 0){
				if(delta < - 4 * RVO_EPSILON * c)return new SuperAgent(null);
				else delta = 0;
			}
		...
		if (d < Vector2.abs(observer.position_ - intersectionPoint) + observer.radius_ + d*Math.Sin(alpha2)) return new SuperAgent(null);
