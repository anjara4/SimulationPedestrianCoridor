...
namespace RVO
{
    ...
    internal abstract class Agent : SuperAgent
    {
        ...
        internal float sharingEffort;
        internal int nbTour=0;
        internal int model_follow_type;

        internal bool isAccFollow = false;
        internal bool isCollisionOccurs = false;
        internal bool isAccFollowGreater = false;
        ...
        */
        public void applymodel()
        {
            ...
            if (follows_)
            {
                ...
                applyFollowingBehavior(acc_follow, sim_.getModel_follow_type(1));
                //applyFollowingBehavior(acc_follow, 1);
            }
        }
        float min_variable(float var_a, float var_b)
        {
            if (var_a < var_b) return var_a;
            else return var_b;
        }
        float max_variable(float var_a, float var_b)
        {
            if (var_a > var_b) return var_a;
            else return var_b;
        }
        ...
        */
        void applyFollowingBehavior(float following_acc, int num_case)
        {
            ...
            Vector2 local_new_acceleration;
            switch (num_case)
            {
                case 1: //Min
                    if (following_acc < local_acceleration.x())
                    {
                        local_new_acceleration = new Vector2(following_acc, local_acceleration.y());
                        acceleration_ = Vector2.rotation(local_new_acceleration, alpha);
                        newVelocity_ = velocity_ + acceleration_ * sim_.getTimeStep();
                        isAccFollowGreater = false;
                        isAccFollow = true;
                    }
                    else
                    {
                        isAccFollowGreater = true;
                        isAccFollow = false;
                    }
                    break;
                case 11: //1bis 
                    float acc;
                    if (following_acc > 0 && local_acceleration.x() > 0)
                    {
                        acc = min_variable(following_acc, local_acceleration.x());
                        if (acc == following_acc)
                        {
                            isAccFollow = true;
                            isAccFollowGreater = false;
                        }
                    ...
                    else if (following_acc < 0 && local_acceleration.x() < 0)
                    {
                        acc = max_variable(following_acc, local_acceleration.x());
                        if (acc == following_acc)
                        {
                            isAccFollow = true;
                            isAccFollowGreater = true;
                    ...
                    {
                        acc = min_variable(Math.Abs(following_acc), Math.Abs(local_acceleration.x()));
                        if (acc == Math.Abs(following_acc))
                        {
                            acc = following_acc;
                            isAccFollow = true;
                            isAccFollowGreater = false;
                        ...
                        {
                            acc = local_acceleration.x();
                            isAccFollow = false;
                            isAccFollowGreater = true;
                        }
                    }

                    local_new_acceleration = new Vector2(acc, local_acceleration.y());
                    acceleration_ = Vector2.rotation(local_new_acceleration, alpha);
                    newVelocity_ = velocity_ + acceleration_ * sim_.getTimeStep();
                    break;
                case 2: //Max
                    ...
                    break;
                case 3: //Medium
                    float acc_mid = (following_acc + local_acceleration.x()) / 2;
                    local_new_acceleration = new Vector2(acc_mid, local_acceleration.y());

                    if (local_acceleration.x() > acc_mid)
                    {
                        isAccFollow = true;
                        isAccFollowGreater = true;
                    }
                    ...
                    break;
                case 33: //Medium
                    float acc_mid_bis = (following_acc + local_acceleration.x()) / 2;

                    if (local_acceleration.x() < acc_mid_bis)
                    {
                        acc_mid_bis = local_acceleration.x();
                        isAccFollow = false;
                        isAccFollowGreater = true;
                    }
                    else
                    ...
                    local_new_acceleration = new Vector2(acc_mid_bis, local_acceleration.y());
                    acceleration_ = Vector2.rotation(local_new_acceleration, alpha);
                    newVelocity_ = velocity_ + acceleration_ * sim_.getTimeStep();
                    break;
                case 4: //Follow
                    ...
                    if (following_acc > local_acceleration.x()) isAccFollowGreater = true;
                    else isAccFollowGreater = false;
                    isAccFollow = true;
                    ...
                    break;
                case 5: //ORCA
                    local_new_acceleration = new Vector2(local_acceleration.x(), local_acceleration.y());
